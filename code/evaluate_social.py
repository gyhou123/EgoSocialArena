...
def evaluate_social():
    ...
    if args.wandb == 1:
        ...
        table = wandb.Table(columns=["story", "perspective", "question", "question_type", "label", "guess","correct"])
    ...
    with open(args.data_dir) as f_in:
        ...
        for index, line in tqdm.tqdm(enumerate(f_in), total=min(len(read_txt(args.data_dir).split('\n')), args.num_probs)):
            ...
            story, question, label, containersa, containersb, containersc, containersd, question_type= fields["STORY"], fields["QUESTION"], fields["答案\nANSWER"], fields["OPTION-A"], fields["OPTION-B"], fields["OPTION-C"], fields["OPTION-D"], fields["能力\nABILITY"]
            if label == 'A':
                label = containersa
            elif label == 'B':
                label = containersb
                ...
                label = containersd
            containers_list = []
            containers_list.append(containersa)
            containers_list.append(containersb)
            containers_list.append(containersc)
            containers_list.append(containersd)
            ...
            #ZhuanHuanModel = Claude_parse('claude-3-5-sonnet-20240620',temperature=args.temperature, verbose=args.verbose)
            ZhuanHuanModel = ChatGPT_parse('gpt-4o',temperature=args.temperature, verbose=args.verbose)
            ZhuanPrompt = prompts_tomi.ZhuanHuanPrompt.format(story=story, question=question, Containers=containers_list)
            Story_zhuanhuan = ZhuanHuanModel.getOutput(ZhuanPrompt)

            baselinePrompt = prompts_tomi.baselineSocialPrompt.format(entire_context = Story_zhuanhuan)
...
"""
            ...
            if args.wandb == 1:
                table.add_data(story, perspective, question, question_type, label, prediction, correct)
